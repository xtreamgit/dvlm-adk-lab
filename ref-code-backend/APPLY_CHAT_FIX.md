...
1. **Open Cloud SQL Console:**
   - Go to: https://console.cloud.google.com/sql/instances/adk-multi-agents-db/overview?project=dvlm-adk-lab
...
3. **Connect to database:**
   ```bash
   gcloud sql connect adk-multi-agents-db --user=adk_app_user --database=adk_agents_db --project=dvlm-adk-lab
   ```
...
```bash
cat <<'EOF' | gcloud sql connect adk-multi-agents-db --user=adk_app_user --database=adk_agents_db --project=dvlm-adk-lab
ALTER TABLE user_sessions ADD COLUMN IF NOT EXISTS message_count INTEGER DEFAULT 0;
ALTER TABLE user_sessions ADD COLUMN IF NOT EXISTS user_query_count INTEGER DEFAULT 0;
