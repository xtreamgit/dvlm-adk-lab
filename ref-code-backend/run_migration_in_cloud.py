...
print('  --image=us-west1-docker.pkg.dev/dvlm-adk-lab/cloud-run-repo1/backend:latest \\')
print('  --region=us-west1 \\')
print('  --project=dvlm-adk-lab \\')
print('  --set-env-vars="DB_TYPE=postgresql,DB_NAME=adk_agents_db,DB_USER=adk_app_user,DB_HOST=/cloudsql/dvlm-adk-lab:us-west1:adk-multi-agents-db,CLOUD_SQL_CONNECTION_NAME=dvlm-adk-lab:us-west1:adk-multi-agents-db" \\')
print('  --set-secrets=DB_PASSWORD=db-password:latest \\')
print('  --set-cloudsql-instances=dvlm-adk-lab:us-west1:adk-multi-agents-db \\')
print('  --command=python3 \\')
print('  --args=-c \\')
print(f"  --args='{migration_script}' \\")
print('  --service-account=backend-sa@dvlm-adk-lab.iam.gserviceaccount.com')
print()
print("Then execute with: gcloud run jobs execute user-sessions-migration --region=us-west1 --project=dvlm-adk-lab")
