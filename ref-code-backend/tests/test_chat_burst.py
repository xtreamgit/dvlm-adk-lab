...
def auth_headers(client: TestClient) -> dict:
    reg_payload = {
        "username": "burst",
        "password": "Bur5t!",
        "full_name": "Burst Tester",
        "email": "burst@example.com",
    }
    r = client.post("/api/auth/register", json=reg_payload)
    assert r.status_code == 200, r.text
    r = client.post("/api/auth/login", json={"username": "burst", "password": "Bur5t!"})
    assert r.status_code == 200, r.text
    token = r.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}


def test_chat_burst_no_5xx_and_history_growth(tmp_path: Path):
    db_file = tmp_path / "users.db"
    client = build_client(db_file)
    headers = auth_headers(client)

    # Create a session
    r = client.post("/api/sessions", headers=headers, json={"name": "B", "preferences": "p"})
    assert r.status_code == 200, r.text
    ...
    for i in range(count):
        r = client.post(f"/api/sessions/{session_id}/chat", headers=headers, json={"message": f"msg {i}"})
        assert r.status_code == 200, r.text
        assert r.json().get("response")

    # History should have 2 entries per message
    r = client.get(f"/api/sessions/{session_id}/history", headers=headers)
    assert r.status_code == 200
    hist = r.json().get("chat_history", [])
    assert len(hist) == count * 2
    # Ensure alternating roles
    for idx in range(0, len(hist), 2):
        assert hist[idx]["role"] == "user"
        assert hist[idx+1]["role"] == "assistant"
