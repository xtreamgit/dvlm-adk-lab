...
def auth_headers(client: TestClient) -> dict:
    reg_payload = {
        "username": "sessadm",
        "password": "SessAdm!",
        "full_name": "Sess Admin",
        "email": "sessadm@example.com",
    }
    r = client.post("/api/auth/register", json=reg_payload)
    assert r.status_code == 200, r.text
    r = client.post("/api/auth/login", json={"username": "sessadm", "password": "SessAdm!"})
    assert r.status_code == 200, r.text
    token = r.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}


def test_session_delete_is_idempotent_and_list_admin_sessions(tmp_path: Path):
    db_file = tmp_path / "users.db"
    client = build_client(db_file)
    headers = auth_headers(client)

    # Create a session
    r = client.post("/api/sessions", headers=headers, json={"name": "S", "preferences": "p"})
    assert r.status_code == 200, r.text
    session_id = r.json()["session_id"]

    # Admin list sessions should include it
    r = client.get("/api/admin/sessions", headers=headers)
    assert r.status_code == 200, r.text
    ...
    # Admin list sessions should no longer include it
    r = client.get("/api/admin/sessions", headers=headers)
    assert r.status_code == 200
    sessions = r.json().get("sessions", [])
