...
def get_auth_headers(client: TestClient) -> dict:
    ...
    r = client.post("/api/auth/login", json={"username": "bob", "password": "S3ssion!"})
    assert r.status_code == 200, r.text
...
def test_session_lifecycle(tmp_path: Path):
    ...
    # Create a session
    r = client.post("/api/sessions", headers=headers, json={"name": "Bob", "preferences": "pref"})
    assert r.status_code == 200, r.text
    session = r.json()
    ...
    # Update profile
    r = client.put(f"/api/sessions/{session_id}/profile", headers=headers, json={"name": "Bob", "preferences": "updated"})
    assert r.status_code == 200, r.text
    assert r.json().get("message") == "Profile updated successfully"

    # Get history (should be empty list)
    r = client.get(f"/api/sessions/{session_id}/history", headers=headers)
    assert r.status_code == 200, r.text
    assert r.json().get("chat_history") == []
