...
def test_expired_token_returns_401(tmp_path: Path):
    db_file = tmp_path / "users.db"
    client = build_client(db_file)

    # Register a user
    reg = {"username": "expuser", "password": "GoodP@ss1", "full_name": "Exp User", "email": "exp@example.com"}
    r = client.post("/api/auth/register", json=reg)
    assert r.status_code == 200

    # Build an expired token using server SECRET_KEY/ALGORITHM
    server = importlib.import_module("backend.src.api.server")
    payload = {
        "sub": "expuser",
    ...
    r = client.get("/api/auth/verify", headers={"Authorization": f"Bearer {expired_token}"})
