...
def build_client(tmp_db_path: Path) -> TestClient:
    ...
    # Ensure fresh import if already loaded in prior tests
...
def auth_headers(client: TestClient) -> dict:
    ...
    r = client.post("/api/auth/login", json={"username": "charlie", "password": "Ch@tP@ss!"})
    assert r.status_code == 200, r.text
    token = r.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}


def test_chat_flow_appends_history(tmp_path: Path):
    db_file = tmp_path / "users.db"
    client = build_client(db_file)
    headers = auth_headers(client)

    # Create a session
    r = client.post("/api/sessions", headers=headers, json={"name": "C", "preferences": "p"})
    assert r.status_code == 200, r.text
    session_id = r.json()["session_id"]

    # Chat
    msg = {"message": "Hello agent"}
    r = client.post(f"/api/sessions/{session_id}/chat", headers=headers, json=msg)
    assert r.status_code == 200, r.text
    data = r.json()
    assert data["response"]

    # History should contain user and assistant entries
    r = client.get(f"/api/sessions/{session_id}/history", headers=headers)
    assert r.status_code == 200, r.text
    hist = r.json()["chat_history"]
    assert len(hist) == 2
