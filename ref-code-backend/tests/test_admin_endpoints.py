...
def get_auth_headers(client: TestClient) -> dict:
    ...
    r = client.post("/api/auth/login", json={"username": "admin1", "password": "Adm1n!"})
    assert r.status_code == 200, r.text
    token = r.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}


def test_admin_users_and_stats(tmp_path: Path):
    db_file = tmp_path / "users.db"
    client = build_client(db_file)
    headers = get_auth_headers(client)

    # Add another user to have some data
    ...
    # Stats
    r = client.get("/api/admin/user-stats", headers=headers)
    assert r.status_code == 200, r.text
    stats = r.json()
